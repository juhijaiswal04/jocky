...
static void generate_node(JkyBuffer *buf, JkyAstNode *node) {
    ...
    switch (node->type) {
        ...
        case AST_WHILE_STMT:
            ...
            break;
        case AST_CALL_EXPR:
            // INTERCEPT: forensics.get_auth_logs(limit) or forensics.get_processes()
            if (node->as.call_expr.callee && node->as.call_expr.callee->type == AST_BINARY_EXPR &&
                ...
                // NEW: Check if it's "forensics.get_processes"
                if (strncmp(dot->as.binary.left->token.start, "forensics", 9) == 0 &&
                    strncmp(dot->as.binary.right->token.start, "get_processes", 13) == 0) {
                    
                    jky_buffer_append(buf, "_jky_helper_print_processes()");
                    return; // Done generating this node
                }
            }
...
void jky_codegen_generate(JkyBuffer *buf, JkyAstNode *root) {
    ...
    jky_buffer_append(buf, "#include <stdlib.h>\n"); // Added for free()
    jky_buffer_append(buf, "#include \"jky_log.h\"\n");
    jky_buffer_append(buf, "#include \"jky_value.h\"\n");
    jky_buffer_append(buf, "#include \"jky_process.h\"\n\n"); // NEW: Process enumeration header
    ...
    jky_buffer_append(buf, "        printf(\"\\n--- COLLECTED LOGS ---\\n%s\\n----------------------\\n\", s->data);\n");
    jky_buffer_append(buf, "        FILE* f = fopen(\"auth_logs.json\", \"w\");\n");
    jky_buffer_append(buf, "        if(f) { fputs(\"\\xEF\\xBB\\xBF\", f); fputs(s->data, f); fclose(f); printf(\"[+] Saved to auth_logs.json\\n\"); }\n");
    jky_buffer_append(buf, "    }\n");
    jky_buffer_append(buf, "}\n\n");

    // NEW: Helper for Process Enumeration
    jky_buffer_append(buf, "void _jky_helper_print_processes() {\n");
    jky_buffer_append(buf, "    char* procs = jky_process_enumerate();\n");
    jky_buffer_append(buf, "    if(procs) {\n");
    jky_buffer_append(buf, "        printf(\"\\n--- COLLECTED PROCESSES ---\\n%s\\n----------------------\\n\", procs);\n");
    jky_buffer_append(buf, "        FILE* f = fopen(\"process_logs.json\", \"w\");\n");
    jky_buffer_append(buf, "        if(f) { \n");
    jky_buffer_append(buf, "            fputs(\"\\xEF\\xBB\\xBF\", f); // UTF-8 BOM\n");
    jky_buffer_append(buf, "            fputs(procs, f); \n");
    jky_buffer_append(buf, "            fclose(f); \n");
    jky_buffer_append(buf, "            printf(\"[+] Saved to process_logs.json\\n\"); \n");
    jky_buffer_append(buf, "        }\n");
    jky_buffer_append(buf, "        free(procs);\n");
    jky_buffer_append(buf, "    }\n");
    jky_buffer_append(buf, "}\n\n");
